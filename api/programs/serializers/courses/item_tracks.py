    ...
    def create(self,data): 
        ...
        if position['position__max'] != None:
            new_item_track = CourseItemTrack.objects.create(
                block=block,course=course, item=CourseItem.objects.create(name=name), position=(position['position__max'] + 1)
            )
        else:
            new_item_track = CourseItemTrack.objects.create(
                block=block,course=course, item=CourseItem.objects.create(name=name), position=0
            )
