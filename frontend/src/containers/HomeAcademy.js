...
import {
  fetchTopics,
  fetchTopicsPagination,
} from "../redux/actions/topics/topics";
...
export default function Home(props) {
  ...
  const handleSearchSubmit = () => {
    history.push({
      pathname: `/academy/${program}/search`,
      state: { search: search },
    });
  };
          ...
          <ImgContainer>
            {programReducer.program.video_presentation ? (
              <>
                {isPlaying ? (
                  <>
                    <VideoPlayer
                      video={{
                        video: programReducer.program.video_presentation,
                      }}
                    />
                  </>
                ) : (
              ...
              <img
                ...
                src={programReducer.program && programReducer.program.picture}
              />
            )}
          </ImgContainer>
          ...
          programReducer.program.are_forum ? (
            <ToolsList>
              {programReducer.program.courses > 0 && (
                <Link to={`/academy/${program}/courses`}>
                  <Tool>
              ...
              {programReducer.program.are_meetups && (
                <Link to={`/academy/${program}/meetups`}>
                  <Tool>
                    <div className="d-flex flex-column justify-content-center align-items-center">
                      <IconContext.Provider
                        value={{
          ...
          {(programReducer.program.are_videos ||
            ...
            <>
              {topicsReducer.topics &&
                topicsReducer.topics.results &&
                topicsReducer.topics.results.length > 0 && (
                  <div className="container">
                    <div className="border-bottom mb-3 pb-2 text-center">
                      <span>Temas</span>
                    </div>
                    <TopicsContainer
                      length={topicsReducer.topics.results.length}
                      ref={main}
                    >
                      {topicsReducer.topics &&
...
const TopicsContainer = styled.div`
  ...
  @media screen and (max-width: 1200px) {
    grid-template-columns: ${(props) =>
      props.length < 4 ? `repeat(${props.length}, 1fr)` : "repeat(3, 1fr)"};
  }
  @media screen and (max-width: 992px) {
    grid-template-columns: ${(props) =>
      props.length < 4 ? `repeat(${props.length}, 1fr)` : "repeat(2, 1fr)"};
  }
