...
import {
  ...
  setCurrentFolder,
  removeCurrentFolder,
} from "../redux/actions/sharedFolders";
import { fetchFiles } from "../redux/actions/sharedFiles";
...
export default function DocsAcademy() {
  const main = useRef();
  const sharedFoldersReducer = useSelector(
    (state) => state.sharedFoldersReducer
  );
  const sharedFilesReducer = useSelector((state) => state.sharedFilesReducer);
  ...
  return (
    <>
      <Main padding ref={main}>
        ...
        <ContainerWrapper>
          <div className=" mb-3">
            {sharedFoldersReducer.current_folders.length > 0 && (
              <ButtonCustom
                type="button"
          ...
          <div className="row">
            <div className="col-12">
              <GridFolders>
                {sharedFoldersReducer.folders &&
                  sharedFoldersReducer.folders.map((folder) => (
                    <DocsItem
                      folder={folder}
                      key={folder.id}
                ...
                {sharedFilesReducer.files &&
                  sharedFilesReducer.files.map((file) => (
                    <DocsItem
                      is_file
                      file={file}
                      key={file.id}
              ...
              {sharedFilesReducer.isLoading ||
                (sharedFoldersReducer.isLoading && <span>Cargando...</span>)}
            </div>
          </div>
        </ContainerWrapper>
      </Main>
