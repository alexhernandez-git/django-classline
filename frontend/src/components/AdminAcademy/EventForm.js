...
export default function EventForm(props) {
  ...
  const meetupStudentsReducer = useSelector(
    (state) => state.meetupStudentsReducer
  );
        ...
        {showStudents ? (
          ...
          <>
            ...
            <MyCKEditor
              ...
              handleEdit={(e) =>
                isEdit
                  ? setClassData({ ...classData, description: e })
                  : setArgs({ ...args, description: e })
              }
            />

            {meetupsReducer.meetup_create_error &&
              meetupsReducer.meetup_create_error.data.description &&
              meetupsReducer.meetup_create_error.data.description.map(
            ...
            {classData.online_class && (
              <>
                ...
                <input
                  ...
                  onChange={(e) =>
                    isEdit
                      ? setClassData({
                          ...classData,
                          videoconference: e.target.value,
                        })
                ...
                {meetupsReducer.meetup_create_error &&
                  ...
                  meetupsReducer.meetup_create_error.data.videoconference.map(
                    (error) => (
                      <small className="d-block text-red">{error}</small>
                    )
                  )}
                {meetupsReducer.meetup_edit_error &&
                  ...
                  meetupsReducer.meetup_edit_error.data.videoconference.map(
                    (error) => (
                      <small className="d-block text-red">{error}</small>
                    )
                  )}
            ...
            {classData.bookable && (
              <>
                ...
                {isEdit &&
                  !meetupStudentsReducer.isLoading &&
                  meetupStudentsReducer.students && (
                    <>
                      <label className="mt-4">
                        Alumnos: {meetupStudentsReducer.students.results.length}
                      </label>
                      <div className="mt-2 mb-2">
                        <ButtonCustom onClick={handleShowStudents}>
                          Ver Alumnos
                        </ButtonCustom>
                      </div>
                    </>
