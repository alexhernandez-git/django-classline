...
import {
  fetchBuyPacksPagination,
  fetchMyPacks,
  fetchPublishedPacks,
  setSelectedPack,
} from "../../redux/actions/buyPacks";
const PacksAdmin = () => {
  ...
  return (
    ...
    >
      <ContainerWrapper>
        <Filters title={showMyPacks ? "Mis Packs" : "Packs Publicados"} />
        <div className="d-flex justify-content-end mb-3">
          {showMyPacks ? (
            <ButtonCustomGradient onClick={handleFetchPublishedPacks}>
              Ir a Packs Publicados
            </ButtonCustomGradient>
          ) : (
            <ButtonCustomGradient
              onClick={() =>
                authReducer.isAuthenticated ? handleFetchMyPacks() : goToLogin()
              }
            >
              Ir a Mis Packs
            </ButtonCustomGradient>
